			...
			<tr>
				...
				<td>
					...
					@if(session('statActHide') != 1)
						...
						<a href="{{ asset('proses/delete/struktur/kpa/'.$tmp->geo_prov_id.'/'.$tmp->geo_kab_id.'/'.$tmp->geo_kec_id.'/'.$tmp->geo_deskel_id.'/'.$tmp->geo_rw_id.'/'.$tmp->geo_rt_id.'/'.$tmp->struk_kpa_id) }}" onclick="return confirm('Apakah anda yakin ingin menghapus data ini?');" class="btn btn-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete"><i class="fa fa-trash"></i></a>
