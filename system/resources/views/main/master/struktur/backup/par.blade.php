...
<div class="modal primary" id="tambahKecamatan" role="dialog" aria-labelledby="tambahKelurahanLabel" >
	<div class="modal-dialog modal-md" role="document">
		<form action="{{ asset('proses/tambah/struktur/par') }}" name="barangbuktiform" enctype="multipart/form-data" method="post">
			<div class="modal-content">
				<div class="modal-header modal-primary">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title modal-primary" id="myModalLabel">Tambah Struktur PAR</h4>
				</div>
				<div class="modal-body">
					...
					<div class="row">
						<div class="form-group col-md-12">
							<label for="nama" class="col-md-4">Nama Struktur PAR</label>
							<div class="col-md-8">
								<input type="text" name="nama_par" id="nama_par" class="form-control" placeholder="Nama Struktur PAR">
							</div>
						</div>
					</div>
				</div>
...
<div class="modal primary" id="editKecamatan" role="dialog" aria-labelledby="tambahKelurahanLabel" >
	<div class="modal-dialog modal-md" role="document">
		<form action="{{ asset('proses/edit/struktur/par') }}" name="barangbuktiform" enctype="multipart/form-data" method="post">
			<div class="modal-content">
				<div class="modal-header modal-primary">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="form-group col-md-12">
							<label for="nama" class="col-md-4">Nama Struktur PAR</label>
							<div class="col-md-8">
								<input type="text" name="edit_id_par" id="edit_id_par" class="form-control hide" placeholder="Nama Struktur PAR">
								<input type="text" name="edit_nama_par" id="edit_nama_par" class="form-control" placeholder="Nama Struktur PAR">
							</div>
						</div>
					</div>
				</div>
						  ...
						  <div class="box-header with-border">
							  <div class="row">
								...
								<div class="col-md-2 col-sm-3 col-xs-6 pull-right">
									<div onclick="addPar({{session('idProvinsi')}})" class="btn-block btn-warning btn" data-toggle="modal" data-target="#tambahKecamatan"><i class="fa fa-plus"></i> Tambah</div>
						  ...
						  <div class="panel-body">
							...
							<table class="table table-striped table-hover">
							  ...
							  <tr>
								...
								<td>
								  <div onclick="detailUser('{{ $tmp->struk_par_id }}')" class="btn btn-primary" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Detail"><i class="fa fa-search"></i></div>
								  <div onclick='editPAR("{{ $tmp->geo_prov_id }}","{{ $tmp->geo_kab_id }}","{{ $tmp->geo_kec_id }}","{{ $tmp->geo_deskel_id }}","{{ $tmp->struk_par_id }}","{{ $tmp->struk_par_nama }}")' class="btn btn-warning" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Edit"><i class="fa fa-edit"></i></div>
								  <a href="{{ asset('proses/delete/struktur/par/'.$tmp->geo_prov_id.'/'.$tmp->geo_kab_id.'/'.$tmp->geo_kec_id.'/'.$tmp->geo_deskel_id.'/'.$tmp->struk_par_id) }}" onclick="return confirm('Apakah anda yakin ingin menghapus data ini?');" class="btn btn-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete"><i class="fa fa-trash"></i></a>
...
$('#kel').change(function(){
	...
	var kec = $('#kec').val();
	var kel = $(this).val();
	changeRWOptionKPU('#id_kecamatan','#id_kelurahan',prov,kab,kec,kel);
});
$('#rw').change(function(){
	var prov = $('#prov').val();
	var kab = $('#kab').val();
	...
	var kel = $('#kel').val();
	var rw = $(this).val();
	location.href="<?php echo url(); ?>/master/struktur/par/"+prov+"/"+kab+"/"+kec+"/"+kel+"/"+rw;
});
...
function editPAR(prov_id,kab_id,kec_id,kel_id,struk_id,struk_nama){
	$('#edit_id_prov').val(prov_id);
	$('#edit_id_kab').val(kab_id);
	$('#edit_id_kec').val(kec_id);
	$('#edit_id_kel').val(kel_id);
	$('#edit_id_par').val(struk_id);
	$('#edit_nama_par').val(struk_nama);
	$('#btnEditKecamatan').click();
}
function addPar(prov_id){
