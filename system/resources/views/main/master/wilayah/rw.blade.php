...
<div class="modal primary" id="tambahRW" role="dialog" aria-labelledby="tambahKelurahanLabel" >
	<div class="modal-dialog modal-md" role="document">
		<form action="{{ asset('proses/tambah/wilayah/rw') }}" name="barangbuktiform" enctype="multipart/form-data" method="post">
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="form-group col-md-12">
							...
							<div class="col-md-8">
								<input type="text" name="nama_rw" id="nama_rw" class="form-control" placeholder="Nama RW">
							</div>
						</div>
					</div>
				</div>
...
<div class="modal primary" id="editRW" role="dialog" aria-labelledby="tambahKelurahanLabel" >
	<div class="modal-dialog modal-md" role="document">
		<form action="{{ asset('proses/edit/wilayah/rw') }}" name="barangbuktiform" enctype="multipart/form-data" method="post">
			<div class="modal-content">
				<div class="modal-header modal-primary">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title modal-primary" style="color:#fff;font-size: 14px;" id="myModalLabel">Tambah Kecamatan</h4>
				</div>
				<div class="modal-body">
					<div class="row">
						<div class="form-group col-md-12">
					...
					<div class="row">
						<div class="form-group col-md-12">
							<label for="nama" class="col-md-4">Nama RW</label>
							<div class="col-md-8">
								<input type="text" name="edit_id_rw" id="edit_id_rw" class="form-control hide" placeholder="Nama RW">
								<input type="text" name="edit_nama_rw" id="edit_nama_rw" class="form-control" placeholder="Nama RW">
							</div>
						</div>
					</div>
				</div>
						  ...
						  <div class="panel-body">
							...
							<table class="table table-striped table-hover">
							  ...
							  <tr>
								...
								<td>
								  <div onclick="getDataDetailWilayah('rw','{{ $tmp->geo_prov_id }}','{{ $tmp->geo_kab_id }}','{{ $tmp->geo_kec_id }}','{{ $tmp->geo_deskel_id }}','{{ $tmp->geo_rw_id }}')" class="btn btn-primary" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Detail"><i class="fa fa-search"></i></div>
								  <div onclick='editRW("{{ $tmp->geo_prov_id }}","{{ $tmp->geo_kab_id }}","{{ $tmp->geo_kec_id }}","{{ $tmp->geo_deskel_id }}","{{ $tmp->geo_rw_id }}","{{ $tmp->geo_rw_nama }}")' class="btn btn-warning" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Edit"><i class="fa fa-edit"></i></div>
								  <a href="{{ asset('proses/delete/wilayah/rw/'.$tmp->geo_prov_id.'/'.$tmp->geo_kab_id.'/'.$tmp->geo_kec_id.'/'.$tmp->geo_deskel_id.'/'.$tmp->geo_rw_id) }}" onclick="return confirm('Apakah anda yakin ingin menhapus data ini?');" class="btn btn-danger" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Delete"><i class="fa fa-trash"></i></a>
								  <div onclick="printUser('{{ $tmp->geo_rw_id }}')" class="btn btn-default" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Print"><i class="fa fa-print"></i></div>
								</td>
							  </tr>
							  @endforeach
							  </tbody>
...
$('#kel').change(function(){
	...
	var kec = $('#kec').val();
	var kel = $(this).val();
	location.href="<?php echo url(); ?>/master/wilayah/rw/"+prov+"/"+kab+"/"+kec+"/"+kel;
});
