...
<td style="width:1%;white-space:nowrap;">
	...
	<div onclick="actionEdit({{$val->r_bio_id}},`{{$val->nama or ''}}`,{{$val->id_anggota or 'null'}},`{{@$val->nama_jabatan?:'-'}}`,{{$val->jabatan_id or 'null'}},{{'{'.(@$obj?:'').'}'}})" class="btn btn-warning" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="Edit"><i class="fa fa-edit"></i></div>
...
@section('table_data')
		...
		<td>{{ $val->nama?:'-' }}</td>
		<td>{{ ($val->gender==1?'L':($val->gender==0?'P':($val->gender==2?'P':'-'))) }}</td>
		<td>{{ $val->no_sk2?:'-' }}</td>
