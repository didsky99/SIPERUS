...
@section('content')
	...
	@section('content_filter_combo')
		@section('indo_combo_fdpp','initial')
...
@section('function')
<script>
	$('#btn-search-dpp').on('click', function(){
		var based_on = $('#search-by').val();
		var keyword = $("[name='keyword']").val();

		/*alert(based_on+" "+keyword);*/

		window.location=("{{ asset('data_pengurus/dpp') }}?based_on="+based_on+"&keyword="+keyword);

		/*$.ajax({
			type : "GET",
			url : "{{ asset('data_pengurus/dpp') }}",
			data : {
				'based_on' : based_on,
				'keyword' : keyword
			},
